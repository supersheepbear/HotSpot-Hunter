...
def _render_ai_analysis(ai_analysis: Any, channel: str) -> str:
    ...
    try:
        # 尝试从 app.ai 导入 AI 分析渲染器
        from app.ai.formatter import get_ai_analysis_renderer
...
def send_to_feishu(
    ...
    *,
    ...
    # 1. 推送热榜内容（如果启用且 report_data 有内容）
    if display_regions.get("HOTLIST", True) and report_data.get("stats"):
        from app.notification.renderer import render_feishu_content
        hotlist_content = render_feishu_content(
            report_data=report_data,
            update_info=update_info,
            mode=mode,
            get_time_func=get_time_func,
            show_new_section=False,  # 重要新闻推送不显示新增部分
        )
        if hotlist_content:
            content_parts.append(hotlist_content)
    
    # 2. 推送AI分析内容（如果启用且有AI分析）
    if display_regions.get("AI_ANALYSIS", True) and ai_analysis and getattr(ai_analysis, "success", False):
        try:
            from app.ai import render_ai_analysis_feishu
        except ImportError:
            def render_ai_analysis_feishu(ai_analysis):
                return ""
        ai_content = render_ai_analysis_feishu(ai_analysis)
        if ai_content:
            if content_parts:
    ...
    if not content_parts:
        print(f"{log_prefix}无推送内容，跳过推送 [{report_type}]")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 检查飞书的响应状态（v2 webhook 返回格式：{"code": 0, "msg": "success"}）
            if result.get("code") == 0 or result.get("StatusCode") == 0:
                print(f"{log_prefix}发送成功 [{report_type}]")
                return True
            else:
                error_msg = result.get("msg") or result.get("StatusMessage", "未知错误")
                error_code = result.get("code") or result.get("StatusCode", "未知")
                print(f"{log_prefix}发送失败 [{report_type}]，错误码：{error_code}，错误信息：{error_msg}")
                print(f"{log_prefix}响应内容：{result}")
                return False
        else:
            print(f"{log_prefix}发送失败 [{report_type}]，状态码：{response.status_code}")
            print(f"{log_prefix}响应内容：{response.text}")
            return False
    ...
    *,
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {
                "title": f"HotSpotHunter 热点分析报告 - {report_type}",
                "text": batch_content,
            },
        }
...
def send_to_email(
    ...
    *,
    ...
    try:
        ...
        subject = f"HotSpotHunter 热点分析报告 - {report_type} - {now.strftime('%m月%d日 %H:%M')}"
