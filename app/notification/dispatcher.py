...
# 临时占位符函数
def parse_multi_account_config(value: str) -> list:
    if not value:
        return []
    return [v.strip() for v in value.split(";") if v.strip()]

def limit_accounts(accounts: list, max_count: int, channel_name: str) -> list:
    if len(accounts) <= max_count:
        return accounts
    print(f"[警告] {channel_name} 配置了 {len(accounts)} 个账号，但限制为 {max_count} 个，将只使用前 {max_count} 个")
    return accounts[:max_count]

def get_account_at_index(accounts: list, index: int, default: str = "") -> str:
    if index < len(accounts):
        return accounts[index]
    return default

def validate_paired_configs(configs: dict, channel_name: str, required_keys: list) -> tuple:
    lengths = [len(configs.get(k, [])) for k in required_keys]
    if not lengths or len(set(lengths)) != 1:
        print(f"[警告] {channel_name} 配置的配对参数数量不一致")
        return False, 0
    return True, lengths[0] if lengths else 0
...
if TYPE_CHECKING:
    from app.ai import AIAnalysisResult
    # AITranslator 暂未迁移
    AITranslator = None
...
class NotificationDispatcher:
    ...
    def _send_email(
        ...
        return send_to_email(
            ...
            report_type=report_type,
            html_file_path=html_file_path or "",
